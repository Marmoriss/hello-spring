...
<jsp:include page="/WEB-INF/views/common/header.jsp">
	...
	<button class="btn btn-block btn-outline-danger" id="btn-corona19">확인</button>
	<div id="corona19-wrapper"></div>
	<script>
	document.querySelector("#btn-corona19").addEventListener('click', (e) => {
		$.ajax({
// 			url : "http://openapi.data.go.kr/openapi/service/rest/Covid19/getCovid19SidoInfStateJson?serviceKey=xG7FbVRiB%2FVRULA5s7YlsBsZOsoP1a%2BS%2BdHFBLAaDX%2FRj3QgOTytvSMvI7EbGXFYK0yIDMpKM97N9%2F9awD9dYg%3D%3D&pageNo=1&numOfRows=10&startCreateDt=20220824&endCreateDt=20220824",
			url : "${pageContext.request.contextPath}/data/corona19.do",
			data : {
			...
			success(data){
				...
				const wrapper = document.querySelector("#corona19-wrapper");
				
				// 제주(Jeju) 	확진자 수
				wrapper.innerHTML = `
					<table class="table" id="tbl-xml-corona">
						<thead>
							<tr>
								<th>지역</th>
								<th>확진자 수</th>
				...
				wrapper.querySelector("#tbl-xml-corona tbody").innerHTML = data.reduce((html, corona) => {
					const {gubun, gubunEn, incDec} = corona;
					return `\${html}
						<tr>
							<td>\${gubun}(\${gubunEn})</td>
							<td>\${incDec}</td>
